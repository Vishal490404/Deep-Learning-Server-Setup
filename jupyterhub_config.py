...
c.JupyterHub.ssl_cert = '/root/srv/jupyterhub/cert.pem'
c.JupyterHub.ssl_key = '/root/srv/jupyterhub/key.pem'
...
# No need of below line ig
# c.JupyterHub.hub_connect_ip = '172.31.0.1'

c.JupyterHub.spawner_class = 'dockerspawner.DockerSpawner'
c.Authenticator.admin_users = {"av", "tejas", "juveriya"}
...
c.DockerSpawner.image = 'jupyterhubbase-py'
c.DockerSpawner.notebook_dir = "/home/user"
c.DockerSpawner.volumes = {"jupyterhub-user-{username}": "/home/user"}
...
# this is a culler first read and then change
import sys
...
#  Default: '/home/av/.local/share/jupyterhub'
# c.JupyterHub.data_files_path = '/home/av/.local/share/jupyterhub'
...
#------------------------------------------------------------------------------
# DockerSpawner(Spawner) configuration
#------------------------------------------------------------------------------
...
#  
#  If specified, users will be presented with a form from which they can select
#  an image to run.
#  
#  If a dictionary, the keys will be the options presented to users and the
#  values the actual images that will be launched.
#  
...
#  The user is accessible as spawner.user. The callable should return a dict or
#  list or None as above.
#  
#  If empty (default), the value from ``image`` is used and any attempt to
#  specify the image via user_options will result in an error.
#  
#  .. versionchanged:: 13
...
# c.DockerSpawner.apply_user_options = None
...
# c.DockerSpawner.auth_state_hook = None

## Volume name
#  
#          The same string-templating applies to this
#          as other volume names.
#  Default: '{prefix}ssl-{username}'
# c.DockerSpawner.certs_volume_name = '{prefix}ssl-{username}'
...
# c.DockerSpawner.consecutive_failure_limit = 0

## Deprecated, use ``DockerSpawner.image``.
#  Default: 'quay.io/jupyterhub/singleuser:5.4'
# c.DockerSpawner.container_image = 'quay.io/jupyterhub/singleuser:5.4'

## Deprecated, use ``DockerSpawner.host_ip``
#  Default: '127.0.0.1'
# c.DockerSpawner.container_ip = '127.0.0.1'

## Deprecated, use ``DockerSpawner.name_template``.
#  Default: ''
# c.DockerSpawner.container_name_template = ''
...
#  Default: ''
# c.DockerSpawner.container_prefix = ''

## 
#  See also: Spawner.cpu_guarantee
# c.DockerSpawner.cpu_guarantee = None

## CPU limit for containers
#  
#  Will set cpu_quota = cpu_limit * cpu_period
#  
#  The default cpu_period of 100ms will be used, unless overridden in
#  extra_host_config.
#  
#  Alternatively to a single float, cpu_limit can also be a callable that takes
#  the spawner as the only argument and returns a float:
#  
...
#  Default: traitlets.Undefined
# c.DockerSpawner.cpu_limit = traitlets.Undefined
...
# c.DockerSpawner.env_keep = ['JUPYTERHUB_SINGLEUSER_APP']
...
#  Default: traitlets.Undefined
# c.DockerSpawner.extra_create_kwargs = traitlets.Undefined

## Additional args to create_host_config for container create.
#  
#  If a callable, will be called with the Spawner as the only argument, must
#  return the same dictionary structure, and may be async.
#  
#  .. versionchanged:: 13
#  
#      Added callable support.
...
## Any callable that accepts a string template and a DockerSpawner instance as
#  parameters in that order and returns a string.
#  
#          Reusable implementations should go in
#  dockerspawner.VolumeNamingStrategy, tests should go in ...
#  Default: None
# c.DockerSpawner.format_volume_name = None

## 
#  See also: Spawner.group_overrides
# c.DockerSpawner.group_overrides = traitlets.Undefined
...
# c.DockerSpawner.hub_connect_url = None
...
#  Default: traitlets.Undefined
# c.DockerSpawner.image_whitelist = traitlets.Undefined

## 
#  See also: Spawner.ip
# c.DockerSpawner.ip = '127.0.0.1'
...
#  Default: traitlets.Undefined
# c.DockerSpawner.mem_limit = traitlets.Undefined
...
#  format_volume_name is in use, you can use {username} in the source or target
#  paths, and it will be replaced with the current user's name.
#  Default: []
...
#  Default: 'busybox:1.30.1'
# c.DockerSpawner.move_certs_image = 'busybox:1.30.1'

## Name of the container or service: with {username}, {imagename}, {prefix},
#  {servername} replacements. {raw_username} can be used for the original, not
#  escaped username (may contain uppercase, special characters). It is important
#  to include {servername} if JupyterHub's "named servers" are enabled
#  (``JupyterHub.allow_named_servers = True``). If the server is named, the
#  default name_template is "{prefix}-{username}--{servername}". If it is
#  unnamed, the default name_template is "{prefix}-{username}".
#  
#  Note: when using named servers, it is important that the separator between
#  {username} and {servername} is not a character that can occur in an escaped
#  {username}, and also not the single escape character '-'.
#  Default: ''
# c.DockerSpawner.name_template = ''

## Run the containers on this docker network. If it is an internal docker
#  network, the Hub should be on the same network, as internal docker IP
...
# c.DockerSpawner.oauth_client_allowed_scopes = traitlets.Undefined

## Allowed roles for oauth tokens.
#  See also: Spawner.oauth_roles
# c.DockerSpawner.oauth_roles = traitlets.Undefined

## 
#  See also: Spawner.options_form
# c.DockerSpawner.options_form = traitlets.Undefined

## 
#  See also: Spawner.options_from_form
# c.DockerSpawner.options_from_form = traitlets.Undefined
...
# c.DockerSpawner.poll_jitter = 0.1

## 
#  See also: Spawner.port
# c.DockerSpawner.port = 0

## If specified, the command will be executed inside the container
#          after starting.
#          Similar to using 'docker exec'
#  Default: False
# c.DockerSpawner.post_start_cmd = False

## 
#  See also: Spawner.post_stop_hook
# c.DockerSpawner.post_stop_hook = None

## 
#  See also: Spawner.pre_spawn_hook
# c.DockerSpawner.pre_spawn_hook = None

## Prefix for container names. See name_template for full container name for a
#  particular user's server.
#  Default: 'jupyter'
# c.DockerSpawner.prefix = 'jupyter'

## 
#  See also: Spawner.progress_ready_hook
# c.DockerSpawner.progress_ready_hook = None
...
#  Choices: any of ['always', 'ifnotpresent', 'never', 'skip'] (case-insensitive)
#  Default: 'ifnotpresent'
# c.DockerSpawner.pull_policy = 'ifnotpresent'

## Map from host file/directory to container file/directory. Volumes specified
#  here will be read-only in the container.
#  
...
# c.DockerSpawner.server_token_scopes = traitlets.Undefined

## List of SSL alt names (list of strings).
#  See also: Spawner.ssl_alt_names
# c.DockerSpawner.ssl_alt_names = traitlets.Undefined

## Whether to include `DNS:localhost`, `IP:127.0.0.1` in alt names
#  See also: Spawner.ssl_alt_names_include_local
# c.DockerSpawner.ssl_alt_names_include_local = True
...
#  Default: None
# c.DockerSpawner.tls = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  Default: None
# c.DockerSpawner.tls_assert_hostname = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  Default: None
# c.DockerSpawner.tls_ca = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  Default: None
# c.DockerSpawner.tls_cert = None
...
## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  Default: None
# c.DockerSpawner.tls_key = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  Default: None
# c.DockerSpawner.tls_verify = None
...
#  
#  instead of an IP address. This should work in general when using docker
#  networks, and must be used when internal_ssl is enabled. It is enabled by
#  default if internal_ssl is enabled.
#  Default: False
# c.DockerSpawner.use_internal_hostname = False

## Enable the usage of the internal docker ip. This is useful if you are running
#  jupyterhub (as a container) and the user containers within the same docker
#  network. E.g. by mounting the docker socket of the host into the jupyterhub
#  container. Default is ``True`` if using a docker network, ``False`` if bridge
#  or host networking is used.
#  Default: False
# c.DockerSpawner.use_internal_ip = False

## Map from host file/directory to container (guest) file/directory mount point
#  and (optionally) a mode. When specifying the guest mount point (bind) for the
#  volume, you may use a dict or str. If a str, then the volume will default to a
#  read-write (mode="rw"). With a dict, the bind is identified by "bind" and the
#  "mode" may be one of "rw" (default), "ro" (read-only), "z" (public/shared
#  SELinux volume label), and "Z" (private/unshared SELinux volume label).
#  
#  If format_volume_name is not set, default_format_volume_name is used for
#  naming volumes. In this case, if you use {username} in either the host or
#  guest file/directory path, it will be replaced with the current user's name.
...
#------------------------------------------------------------------------------
# SwarmSpawner(DockerSpawner) configuration
#------------------------------------------------------------------------------
## A Spawner for JupyterHub that runs each user's server in a separate docker
#  service

## 
#  See also: DockerSpawner.allowed_images
# c.SwarmSpawner.allowed_images = {}

## 
#  See also: Spawner.apply_user_options
# c.SwarmSpawner.apply_user_options = None
...
## 
#  See also: Spawner.auth_state_hook
# c.SwarmSpawner.auth_state_hook = None

## Volume name
#  See also: DockerSpawner.certs_volume_name
# c.SwarmSpawner.certs_volume_name = '{prefix}ssl-{username}'

## Extra keyword arguments to pass to the docker.Client constructor.
#  See also: DockerSpawner.client_kwargs
# c.SwarmSpawner.client_kwargs = {}
...
## 
#  See also: Spawner.consecutive_failure_limit
# c.SwarmSpawner.consecutive_failure_limit = 0

## Deprecated, use ``DockerSpawner.image``.
#  See also: DockerSpawner.container_image
# c.SwarmSpawner.container_image = 'quay.io/jupyterhub/singleuser:5.4'

## Deprecated, use ``DockerSpawner.host_ip``
#  See also: DockerSpawner.container_ip
# c.SwarmSpawner.container_ip = '127.0.0.1'

## Deprecated, use ``DockerSpawner.name_template``.
#  See also: DockerSpawner.container_name_template
# c.SwarmSpawner.container_name_template = ''

## Deprecated, use ``DockerSpawner.port``.
#  See also: DockerSpawner.container_port
# c.SwarmSpawner.container_port = 8888

## Deprecated, use ``DockerSpawner.prefix``.
#  See also: DockerSpawner.container_prefix
# c.SwarmSpawner.container_prefix = ''

## 
#  See also: Spawner.cpu_guarantee
# c.SwarmSpawner.cpu_guarantee = None

## 
#  See also: DockerSpawner.cpu_limit
# c.SwarmSpawner.cpu_limit = traitlets.Undefined
...
## 
#  See also: Spawner.disable_user_config
# c.SwarmSpawner.disable_user_config = False

## 
#  See also: Spawner.env_keep
# c.SwarmSpawner.env_keep = ['JUPYTERHUB_SINGLEUSER_APP']
...
## Override escaping with any callable of the form escape(str)->str
#  See also: DockerSpawner.escape
# c.SwarmSpawner.escape = None
...
## Additional args to pass for container create
#  See also: DockerSpawner.extra_create_kwargs
# c.SwarmSpawner.extra_create_kwargs = traitlets.Undefined
...
## 
#  See also: DockerSpawner.extra_host_config
# c.SwarmSpawner.extra_host_config = traitlets.Undefined
...
#  Default: {}
# c.SwarmSpawner.extra_task_spec = {}

## Any callable that accepts a string template and a DockerSpawner instance as
#  parameters in that order and returns a string.
#  See also: DockerSpawner.format_volume_name
# c.SwarmSpawner.format_volume_name = None

## 
#  See also: Spawner.group_overrides
# c.SwarmSpawner.group_overrides = traitlets.Undefined

## The ip address on the host on which to expose the container's port
#  See also: DockerSpawner.host_ip
# c.SwarmSpawner.host_ip = '127.0.0.1'

## 
#  See also: Spawner.http_timeout
# c.SwarmSpawner.http_timeout = 30

## 
#  See also: Spawner.hub_connect_url
# c.SwarmSpawner.hub_connect_url = None

## DEPRECATED since JupyterHub 0.8. Use c.JupyterHub.hub_connect_ip.
#  See also: DockerSpawner.hub_ip_connect
# c.SwarmSpawner.hub_ip_connect = ''

## The image to use for single-user servers.
#  See also: DockerSpawner.image
# c.SwarmSpawner.image = 'quay.io/jupyterhub/singleuser:5.4'

## Deprecated, use ``DockerSpawner.allowed_images``.
#  See also: DockerSpawner.image_whitelist
# c.SwarmSpawner.image_whitelist = traitlets.Undefined

## 
#  See also: Spawner.ip
# c.SwarmSpawner.ip = '127.0.0.1'

## 
#  See also: DockerSpawner.links
# c.SwarmSpawner.links = {}

## 
#  See also: Spawner.mem_guarantee
# c.SwarmSpawner.mem_guarantee = None

## 
#  See also: DockerSpawner.mem_limit
# c.SwarmSpawner.mem_limit = traitlets.Undefined

## The image used to stage internal SSL certificates.
#  See also: DockerSpawner.move_certs_image
# c.SwarmSpawner.move_certs_image = 'busybox:1.30.1'

## 
#  See also: DockerSpawner.name_template
# c.SwarmSpawner.name_template = ''

## 
#  See also: DockerSpawner.network_name
# c.SwarmSpawner.network_name = 'bridge'
...
## Allowed scopes for oauth tokens issued by this server's oauth client.
#  See also: Spawner.oauth_client_allowed_scopes
# c.SwarmSpawner.oauth_client_allowed_scopes = traitlets.Undefined

## Allowed roles for oauth tokens.
#  See also: Spawner.oauth_roles
# c.SwarmSpawner.oauth_roles = traitlets.Undefined

## 
#  See also: Spawner.options_form
# c.SwarmSpawner.options_form = traitlets.Undefined

## 
#  See also: Spawner.options_from_form
# c.SwarmSpawner.options_from_form = traitlets.Undefined
...
## 
#  See also: Spawner.poll_jitter
# c.SwarmSpawner.poll_jitter = 0.1

## 
#  See also: Spawner.port
# c.SwarmSpawner.port = 0

## If specified, the command will be executed inside the container
#  See also: DockerSpawner.post_start_cmd
# c.SwarmSpawner.post_start_cmd = False

## 
#  See also: Spawner.post_stop_hook
# c.SwarmSpawner.post_stop_hook = None

## 
#  See also: Spawner.pre_spawn_hook
# c.SwarmSpawner.pre_spawn_hook = None

## 
#  See also: DockerSpawner.prefix
# c.SwarmSpawner.prefix = 'jupyter'

## 
#  See also: Spawner.progress_ready_hook
# c.SwarmSpawner.progress_ready_hook = None

## The policy for pulling the user docker image.
#  See also: DockerSpawner.pull_policy
# c.SwarmSpawner.pull_policy = 'ifnotpresent'

## 
#  See also: DockerSpawner.read_only_volumes
# c.SwarmSpawner.read_only_volumes = {}

## Deprecated, use ``DockerSpawner.remove``.
#  See also: DockerSpawner.remove_containers
# c.SwarmSpawner.remove_containers = False

## The list of scopes to request for $JUPYTERHUB_API_TOKEN
#  See also: Spawner.server_token_scopes
# c.SwarmSpawner.server_token_scopes = traitlets.Undefined

## List of SSL alt names (list of strings).
#  See also: Spawner.ssl_alt_names
# c.SwarmSpawner.ssl_alt_names = traitlets.Undefined

## Whether to include `DNS:localhost`, `IP:127.0.0.1` in alt names
#  See also: Spawner.ssl_alt_names_include_local
# c.SwarmSpawner.ssl_alt_names_include_local = True
...
## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls
# c.SwarmSpawner.tls = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_assert_hostname
# c.SwarmSpawner.tls_assert_hostname = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_ca
# c.SwarmSpawner.tls_ca = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_cert
# c.SwarmSpawner.tls_cert = None

## Arguments to pass to docker TLS configuration.
#  See also: DockerSpawner.tls_config
# c.SwarmSpawner.tls_config = {}

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_key
# c.SwarmSpawner.tls_key = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_verify
# c.SwarmSpawner.tls_verify = None

## Deprecated. Docker env variables are always used if present.
#  See also: DockerSpawner.use_docker_client_env
# c.SwarmSpawner.use_docker_client_env = True

## 
#  See also: DockerSpawner.use_internal_hostname
# c.SwarmSpawner.use_internal_hostname = False

## 
#  See also: DockerSpawner.use_internal_ip
# c.SwarmSpawner.use_internal_ip = False

## Use this driver for mounting the notebook volumes. Note that this driver must
#  support multiple hosts in order for it to work across the swarm. For a list of
#  possible drivers, see
#  https://docs.docker.com/engine/extend/legacy_plugins/#volume-plugins
#  Default: ''
# c.SwarmSpawner.volume_driver = ''

## Configuration options for the multi-host volume driver.
#  Default: {}
# c.SwarmSpawner.volume_driver_options = {}

## 
#  See also: DockerSpawner.volumes
# c.SwarmSpawner.volumes = {}

#------------------------------------------------------------------------------
# SystemUserSpawner(DockerSpawner) configuration
#------------------------------------------------------------------------------
## 
#  See also: DockerSpawner.allowed_images
# c.SystemUserSpawner.allowed_images = {}

## 
#  See also: Spawner.apply_user_options
# c.SystemUserSpawner.apply_user_options = None

## 
#  See also: Spawner.args
# c.SystemUserSpawner.args = []

## 
#  See also: Spawner.auth_state_hook
# c.SystemUserSpawner.auth_state_hook = None

## Volume name
#  See also: DockerSpawner.certs_volume_name
# c.SystemUserSpawner.certs_volume_name = '{prefix}ssl-{username}'

## Extra keyword arguments to pass to the docker.Client constructor.
#  See also: DockerSpawner.client_kwargs
# c.SystemUserSpawner.client_kwargs = {}
...
## 
#  See also: Spawner.consecutive_failure_limit
# c.SystemUserSpawner.consecutive_failure_limit = 0

## Deprecated, use ``DockerSpawner.image``.
#  See also: DockerSpawner.container_image
# c.SystemUserSpawner.container_image = 'quay.io/jupyterhub/singleuser:5.4'

## Deprecated, use ``DockerSpawner.host_ip``
#  See also: DockerSpawner.container_ip
# c.SystemUserSpawner.container_ip = '127.0.0.1'

## Deprecated, use ``DockerSpawner.name_template``.
#  See also: DockerSpawner.container_name_template
# c.SystemUserSpawner.container_name_template = ''
...
## Deprecated, use ``DockerSpawner.prefix``.
#  See also: DockerSpawner.container_prefix
# c.SystemUserSpawner.container_prefix = ''

## 
#  See also: Spawner.cpu_guarantee
# c.SystemUserSpawner.cpu_guarantee = None

## 
#  See also: DockerSpawner.cpu_limit
# c.SystemUserSpawner.cpu_limit = traitlets.Undefined

## Enable debug-logging of the single-user server
#  See also: Spawner.debug
# c.SystemUserSpawner.debug = False

## 
#  See also: Spawner.default_url
# c.SystemUserSpawner.default_url = ''

## 
#  See also: Spawner.disable_user_config
# c.SystemUserSpawner.disable_user_config = False

## 
#  See also: Spawner.env_keep
# c.SystemUserSpawner.env_keep = ['JUPYTERHUB_SINGLEUSER_APP']
...
## Override escaping with any callable of the form escape(str)->str
#  See also: DockerSpawner.escape
# c.SystemUserSpawner.escape = None

## Additional args to pass for container create
#  See also: DockerSpawner.extra_create_kwargs
# c.SystemUserSpawner.extra_create_kwargs = traitlets.Undefined

## 
#  See also: DockerSpawner.extra_host_config
# c.SystemUserSpawner.extra_host_config = traitlets.Undefined

## Any callable that accepts a string template and a DockerSpawner instance as
#  parameters in that order and returns a string.
#  See also: DockerSpawner.format_volume_name
# c.SystemUserSpawner.format_volume_name = None

## 
#  See also: Spawner.group_overrides
# c.SystemUserSpawner.group_overrides = traitlets.Undefined

## Mode for bind mount propagation for home directory.
#  
#  Requires docker-py 7.0.
#  
#  See https://docs.docker.com/engine/storage/bind-mounts/#configure-bind-
#  propagation
#  
#  .. versionadded:: 14
#  Default: ''
# c.SystemUserSpawner.homedir_bind_propagation = ''

## Format string for the path to the user's home directory on the host. The
#  format string should include a `username` variable, which will be formatted
#  with the user's username.
#  
#  If the string is empty or `None`, the user's home directory will be looked up
#  via the `pwd` database.
#  Default: '/home/{username}'
# c.SystemUserSpawner.host_homedir_format_string = '/home/{username}'

## The ip address on the host on which to expose the container's port
#  See also: DockerSpawner.host_ip
# c.SystemUserSpawner.host_ip = '127.0.0.1'

## 
#  See also: Spawner.http_timeout
# c.SystemUserSpawner.http_timeout = 30

## 
#  See also: Spawner.hub_connect_url
# c.SystemUserSpawner.hub_connect_url = None

## DEPRECATED since JupyterHub 0.8. Use c.JupyterHub.hub_connect_ip.
#  See also: DockerSpawner.hub_ip_connect
# c.SystemUserSpawner.hub_ip_connect = ''

## The image to use for single-user servers.
#  See also: DockerSpawner.image
# c.SystemUserSpawner.image = 'quay.io/jupyterhub/singleuser:5.4'

## Format string for the path to the user's home directory inside the image.  The
#  format string should include a `username` variable, which will be formatted
#  with the user's username.
#  Default: '/home/{username}'
...
## Deprecated, use ``DockerSpawner.allowed_images``.
#  See also: DockerSpawner.image_whitelist
# c.SystemUserSpawner.image_whitelist = traitlets.Undefined

## 
#  See also: Spawner.ip
# c.SystemUserSpawner.ip = '127.0.0.1'

## 
#  See also: DockerSpawner.links
# c.SystemUserSpawner.links = {}

## 
#  See also: Spawner.mem_guarantee
# c.SystemUserSpawner.mem_guarantee = None

## 
#  See also: DockerSpawner.mem_limit
# c.SystemUserSpawner.mem_limit = traitlets.Undefined

## 
#  See also: DockerSpawner.mounts
# c.SystemUserSpawner.mounts = []

## The image used to stage internal SSL certificates.
#  See also: DockerSpawner.move_certs_image
# c.SystemUserSpawner.move_certs_image = 'busybox:1.30.1'

## 
#  See also: DockerSpawner.name_template
# c.SystemUserSpawner.name_template = ''
...
## 
#  See also: Spawner.notebook_dir
# c.SystemUserSpawner.notebook_dir = ''

## Allowed scopes for oauth tokens issued by this server's oauth client.
#  See also: Spawner.oauth_client_allowed_scopes
# c.SystemUserSpawner.oauth_client_allowed_scopes = traitlets.Undefined

## Allowed roles for oauth tokens.
#  See also: Spawner.oauth_roles
# c.SystemUserSpawner.oauth_roles = traitlets.Undefined

## 
#  See also: Spawner.options_form
# c.SystemUserSpawner.options_form = traitlets.Undefined

## 
#  See also: Spawner.options_from_form
# c.SystemUserSpawner.options_from_form = traitlets.Undefined

## 
#  See also: Spawner.poll_interval
# c.SystemUserSpawner.poll_interval = 30

## 
#  See also: Spawner.poll_jitter
# c.SystemUserSpawner.poll_jitter = 0.1

## 
#  See also: Spawner.port
# c.SystemUserSpawner.port = 0

## If specified, the command will be executed inside the container
#  See also: DockerSpawner.post_start_cmd
# c.SystemUserSpawner.post_start_cmd = False

## 
#  See also: Spawner.post_stop_hook
# c.SystemUserSpawner.post_stop_hook = None

## 
#  See also: Spawner.pre_spawn_hook
# c.SystemUserSpawner.pre_spawn_hook = None

## 
#  See also: DockerSpawner.prefix
# c.SystemUserSpawner.prefix = 'jupyter'

## 
#  See also: Spawner.progress_ready_hook
# c.SystemUserSpawner.progress_ready_hook = None

## The policy for pulling the user docker image.
#  See also: DockerSpawner.pull_policy
# c.SystemUserSpawner.pull_policy = 'ifnotpresent'

## 
#  See also: DockerSpawner.read_only_volumes
# c.SystemUserSpawner.read_only_volumes = {}

## 
#  See also: DockerSpawner.remove
# c.SystemUserSpawner.remove = False

## Deprecated, use ``DockerSpawner.remove``.
#  See also: DockerSpawner.remove_containers
# c.SystemUserSpawner.remove_containers = False
...
#          .. versionadded:: 0.12
#  Default: False
# c.SystemUserSpawner.run_as_root = False

## The list of scopes to request for $JUPYTERHUB_API_TOKEN
#  See also: Spawner.server_token_scopes
# c.SystemUserSpawner.server_token_scopes = traitlets.Undefined

## List of SSL alt names (list of strings).
#  See also: Spawner.ssl_alt_names
# c.SystemUserSpawner.ssl_alt_names = traitlets.Undefined

## Whether to include `DNS:localhost`, `IP:127.0.0.1` in alt names
#  See also: Spawner.ssl_alt_names_include_local
# c.SystemUserSpawner.ssl_alt_names_include_local = True

## 
#  See also: Spawner.start_timeout
# c.SystemUserSpawner.start_timeout = 60

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls
# c.SystemUserSpawner.tls = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_assert_hostname
# c.SystemUserSpawner.tls_assert_hostname = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_ca
# c.SystemUserSpawner.tls_ca = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_cert
# c.SystemUserSpawner.tls_cert = None

## Arguments to pass to docker TLS configuration.
#  See also: DockerSpawner.tls_config
# c.SystemUserSpawner.tls_config = {}

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_key
# c.SystemUserSpawner.tls_key = None

## Deprecated, use ``DockerSpawner.tls_config`` dict to set any TLS options.
#  See also: DockerSpawner.tls_verify
# c.SystemUserSpawner.tls_verify = None

## Deprecated. Docker env variables are always used if present.
#  See also: DockerSpawner.use_docker_client_env
# c.SystemUserSpawner.use_docker_client_env = True

## 
#  See also: DockerSpawner.use_internal_hostname
# c.SystemUserSpawner.use_internal_hostname = False

## 
#  See also: DockerSpawner.use_internal_ip
# c.SystemUserSpawner.use_internal_ip = False
